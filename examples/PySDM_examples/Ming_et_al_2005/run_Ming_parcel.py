...
def run_parcel(
    ...
    # dz to dt
    dt = dz / w
    n_steps = int(np.ceil(z_max / dz))

    min_radius_mean = aerosol.modes[0]["spectrum"].m_mode
    max_radius_mean = aerosol.modes[-1]["spectrum"].m_mode
    dry_radius_bin_edges = np.geomspace(
        min_radius_mean / 10 * si.m, max_radius_mean * 10 * si.m, 40, endpoint=False
    )
    products = (
